...
import { computeHealthSnapshot, computeLoadBalanceScore, healthLevel, ORG_HEALTH_WEIGHTS } from "../lib/orgHealth";
...
export default function WhatIfPage({
  ...
  // 活躍卡點 — 只列 2σ/3σ 卡點，與首頁「本週優先改善」口徑一致
  // 中/低風險不是真正的卡點，列上來只會讓清單變長、稀釋管理焦點
  const activeBlockers = useMemo(() => {
    return blockers
      .filter((b) => b.status !== "resolved")
      .map((b) => ({ blocker: b, ana: analyzeBlockerRecord(b, blockers, history) }))
      .filter(({ ana }) => ana.sigmaLevel === "critical" || ana.sigmaLevel === "warning")
      .sort((a, b) => b.ana.zScore - a.ana.zScore || (b.ana.percentile || 0) - (a.ana.percentile || 0));
  }, [blockers, history]);
  ...
  const projected = useMemo(() => {
    ...
    const overall = +(projectedBase.overall + (loadBalance - projectedBase.loadBalance) * ORG_HEALTH_WEIGHTS.loadBalance).toFixed(1);
  ...
  const totalChanges =
    ...
    Object.values(scenario.loadRelief).filter((v) => (v as number) > 0).length;
        ...
        <Card className="p-5 rounded-2xl">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] text-slate-400">
              已調整 {Object.values(scenario.loadRelief).filter((v) => (v as number) > 0).length} 人
            </span>
          </div>
